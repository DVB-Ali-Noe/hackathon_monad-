...
import { nextLeaderboardRival, parseLeaderboard } from '../utils/leaderboard.ts'
...
test('la cible est le score immédiatement supérieur, indépendamment de l’ordre', () => {
  ...
  assert.deepEqual(nextLeaderboardRival(entries, 100), { ...entries[1], gap: 200n })
  assert.deepEqual(nextLeaderboardRival(entries, 299), { ...entries[1], gap: 1n })
  assert.deepEqual(entries, before)
})

test('atteindre un score change de cible, dépasser plusieurs scores choisit le suivant', () => {
  assert.equal(nextLeaderboardRival(entries, 300).playerId, 'middle')
  assert.equal(nextLeaderboardRival(entries, 650).playerId, 'leader')
  assert.equal(nextLeaderboardRival(entries, 1000), null)
  assert.equal(nextLeaderboardRival(entries, 1200), null)
  assert.equal(nextLeaderboardRival([], 0), null)
})
...
test('les ex æquo conservent une cible stable même si le serveur change leur ordre', () => {
  ...
  assert.equal(nextLeaderboardRival(tied, 0).playerId, 'another')
  assert.equal(nextLeaderboardRival([...tied].reverse(), 0).playerId, 'another')
})
...
test('les grands scores on-chain et leur écart restent exacts', () => {
  ...
  assert.deepEqual(nextLeaderboardRival(large, 20), { ...large[1], gap: 9007199254740973n })
})
